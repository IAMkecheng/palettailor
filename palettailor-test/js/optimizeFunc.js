function doColorization() {
    ...
    // check the palette is discriminable enough
    let min_discirminability_score = 100000000;
    for (let i = 0; i < used_palette.length; i++) {
        for (let j = i + 1; j < used_palette.length; j++) {
            let color_dis = d3_ciede2000(d3.lab(used_palette[i]), d3.lab(used_palette[j]));
            min_discirminability_score = (min_discirminability_score > color_dis) ? color_dis : min_discirminability_score;
        }
    }
    // push into the statics array
    // statics_time.push([tmp_class_number_count, time_consumed, min_discirminability_score.toFixed(2), used_palette])
    // console.log(tmp_class_number_count, time_consumed, min_discirminability_score.toFixed(2), used_palette);
